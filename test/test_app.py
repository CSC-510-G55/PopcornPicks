...
def test_recent_friend_movies(mock_db_and_user):
    """
    Test the /getRecentFriendMovies route when getting recent movies for a friend.
    """
    # Mock data for a friend
    friend_data = {
        "_id": "5fcb45e6b16c401b8ad45672",  # Mock friend ID
        "name": "Friend Name"
    }
    
    # Mock the result of get_recent_friend_movies
    mocked_movies = [
        {"title": "Movie 1", "release_date": "2024-01-01"},
        {"title": "Movie 2", "release_date": "2024-02-01"}
    ]
    
    with app.test_client() as client:
        with patch("backend.recommenderapp.app.get_recent_friend_movies", return_value=mocked_movies):
            # Simulate a POST request with friend_id in the data
            response = client.post("/getRecentFriendMovies", json={"friend_id": friend_data})
            
            # Assert that the response is a 200 OK
            assert response.status_code == 200
            assert "Movie 1" in str(response.data)  # Check if mocked movie is in the response
            assert "Movie 2" in str(response.data)  # Check if mocked movie is in the response
            assert len(response.json) == 2  # Ensure that two movies are returned

def test_no_recent_movies(client):
    # Arrange: Mock data for a friend
    friend_data = {
        "_id": "5fcb45e6b16c401b8ad45672",  # Mock friend ID
        "name": "Friend Name"
    }
    
    # Mock get_recent_friend_movies to return an empty list
    mocked_movies = []

    with patch("backend.recommenderapp.app.get_recent_friend_movies", return_value=mocked_movies):
        # Act: Simulate a POST request with friend_id
        response = client.post("/getRecentFriendMovies", json={"friend_id": friend_data})

        # Assert: Check if the response is 200 OK and returns an empty list
        assert response.status_code == 200
        assert response.json == []  # Expect an empty list for recent movies

def test_get_recent_friend_movies_failure(client):
    # Arrange: Mock data for a friend
    friend_data = {
        "_id": "5fcb45e6b16c401b8ad45672",  # Mock friend ID
        "name": "Friend Name"
    ...
    # Simulate a database failure (mock the function to raise an exception)
    with patch("backend.recommenderapp.app.get_recent_friend_movies", side_effect=Exception("Database error")):
        # Act: Simulate a POST request with friend_id
        response = client.post("/getRecentFriendMovies", json={"friend_id": friend_data})

        # Assert: Check if the response is 500 Internal Server Error
        assert response.status_code == 500

def test_large_number_of_movies(client):
    # Arrange: Mock data for a friend
    friend_data = {
        "_id": "5fcb45e6b16c401b8ad45672",  # Mock friend ID
        "name": "Friend Name"
    ...
    # Mock get_recent_friend_movies to return a large number of movies
    mocked_movies = [{"title": f"Movie {i}", "release_date": f"2024-01-{i:02d}"} for i in range(1, 101)]

    with patch("backend.recommenderapp.app.get_recent_friend_movies", return_value=mocked_movies):
        # Act: Simulate a POST request with friend_id
        response = client.post("/getRecentFriendMovies", json={"friend_id": friend_data})

        # Assert: Check if the response is 200 OK and that 100 movies are returned
        assert response.status_code == 200
        assert len(response.json) == 100  # Expect 100 movies returned
        assert "Movie 1" in str(response.data)  # Check if the first movie is included
        assert "Movie 100" in str(response.data)  # Check if the 100th movie is included
