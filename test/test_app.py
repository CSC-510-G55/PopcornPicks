...
def mock_db_and_user():
    with patch("backend.recommenderapp.app.db") as mock_db:
        mock_db.users.find_one = MagicMock(
            return_value={"email": "testuser@example.com"}
        )
...
def test_get_friends(mock_db_and_user):
    ...
    with app.test_client() as client:
        ...
        with patch("backend.recommenderapp.app.get_friends") as mock_get_friends:
            ...
            mock_get_friends.assert_called_with(
                mock_db_and_user, "673e820d3f9e32b77366db00"
            )  # Hardcoded user ID
...
def test_get_friends_db_failure(client):
    ...
    with patch(
        "backend.recommenderapp.app.get_friends",
...
def test_feedback(mock_db_and_user):
    ...
    feedback_data = {"Liked": [], "Disliked": [], "Yet to Watch": []}
    ...
    with app.test_client() as client:
        with patch("backend.recommenderapp.app.send_email_to_user") as mock_send_email:
            with patch(
                "backend.recommenderapp.app.beautify_feedback_data",
                return_value=feedback_data,
            ):
                # Simulate the request
                response = client.post(
                    "/feedback", json={"message": "Amazing app!", "rating": 5}
                )

                # Ensure the email function is called with the correct processed data
                mock_send_email.assert_called_with(
                    "testuser@example.com", feedback_data
                )
...
def test_feedback_email_failure(client):
    ...
    feedback_data = {"message": "Amazing app!", "rating": 5}

    # Simulate the case where sending email fails (raise an exception)
    with patch(
        "backend.recommenderapp.app.send_email_to_user",
...
def test_beautify_feedback_data_failure(client):
    ...
    with patch(
        "backend.recommenderapp.app.beautify_feedback_data",
        side_effect=Exception("Invalid feedback data"),
    ):
...
def test_username_route(mock_get_username, client):
    ...
    assert (
        response.data == b"test_user"
    )  # The response should be the username "test_user"
