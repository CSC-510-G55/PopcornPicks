...
const Lists = () => {
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		try {
			const res = await axios.post(`${API_BASE_URL}/lists`, data);
			setMovies([]);
	...
	return (
		<div className='container'>
			<div className='mt-3'>
				...
				<ul className='list-group'>
					{movies.map((movie, index) => (
						<li key={index} className='list-group-item d-flex justify-content-between align-items-center'>
							{movie}
							<button className='btn btn-danger btn-sm' onClick={() => handleRemoveMovie(movie)}>
								Remove
							</button>
						</li>
					))}
				...
				/>
				<button type='submit' onClick={handleSubmit} className='btn btn-primary text-white'>
