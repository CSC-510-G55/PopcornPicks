...
def recommend_for_new_user(user_rating, user_history, user_id, client):
    ...
    surprise_df["user"] = surprise_df["user"].apply(lambda x: int(x, 16) if pd.notnull(x) else None)
    ...
    selected_movies = movies[movies["title"].isin([movie["title"] for movie in user_rating])]
    ...
    enriched_movies_genres = enriched_movies_genres.reindex(columns=avg_genre_vector.index, fill_value=0)
    
    enriched_movies["genre_similarity"] = enriched_movies_genres.dot(avg_genre_vector)
    
    avg_runtime = selected_movies["runtime"].mean()
    
    enriched_movies["runtime_similarity"] = 1 - abs(enriched_movies["runtime"] - avg_runtime) / enriched_movies["runtime"].max()
